...
In VQL, declaring the types of parameters and variables is not mandatory. As
such, during the production of a model state, the framework has to infer the
types of entities that do not have an explicitly declared type. During this
process, \texttt{getJvmType} is called multiple times. Let us see, how this
function works.
...
beginning of each validation process. To do so, I've added cache clearing to the
start of the \texttt{installDerivedState} function.
