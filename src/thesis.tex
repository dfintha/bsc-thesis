...
%% TODO: Explain pattern matching on models
...
\begin{itemize}
    ...
    \item{linker}
    \item{typechecker}
    \item{editing support in any editor supporting the Language Server Protocol}
\end{itemize}

It should also be noted, that some internal parts of VQL were implemented using
the Xtend language, which is also part of the Xtext project, implemented in
Xtext itself. Xtend is a Java dialect, with some modern language elements, such
as macros, lambdas, and operator overloading\cite{xtend}.
...
compile and run the relevant components of VIATRA Query their dependencies,
closing any unrelated functionalities, so they won't distract my investigation.
...
After having a proper candidate, I had to create a VIATRA project around it, and
remove duplicated pattern code from it, which existed because of its generated
nature. The finished query did not contain any errors, but caused massive
slowdowns -- perfect for my cause.

Before measuring anything with a profiler, I've conducted some manual testing,
which consisted of smaller editing sessions. During these tests I've noticed,
that the critical part of the editing session is during the Xtext validation
process, which causes a 5-10 second hangup after a few edits in the file.
...
%% TODO: Run tests, write down test results
...
\subsection{Looking for a more fine-grained cache}
To make such a change, I had to look for a more fine-grained cache than Xtext's
\texttt{OnChangeEvictingCache}, which caches complete files, and discards cached
information when the file is changed. But sadly, after spending hours looking
for such solution, I did not find any. On a developer forum, it was also
explicitly stated to be nonexistent\cite{xtext-fine-grained-caching}.

\section{Conclusion}
%% TODO: Conclude my work
%% TODO: Add diagram for run time changes
...
%% TODO: Describe the integration process

\chapter{Summary}
%% TODO: Summarize work

\section{Improvement opportunities}
%% TODO: Add improvement opportunities
